...
def half_gram_matrix(gram):
    a = np.triu(gram.cpu().detach().numpy(),0)
...
def gram_value(features):

    gram_value_features = []
    for x in features:
        x = x.view(1,-1)
        gram_value_features.append(x)
    gram_value_features = torch.cat(gram_value_features,dim=-1)
    #print(gram_value_features.size())  #torch.Size([1, 348160])
    return gram_value_features
